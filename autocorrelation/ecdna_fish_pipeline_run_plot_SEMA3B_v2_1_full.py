### ecdna_fish_pipeline_run_plot_SEMA3B v1
...
# STEP 1: USER CONFIG
# =========================================================
PATHA = Path(r"D:\02.project\18.ecDNA\02.code\v0.1.1\autocorrelation\output_ws\SEMA3B")
GROUP = "SEMA3B"
OUT_PREFIX = "merged"
...
# 自定义总输出目录（不再写到 PATHA）
# =========================
OUT_DIR = Path(r"D:\02.project\18.ecDNA\02.code\v0.1.1\autocorrelation\output_ws_plot\SEMA3B_MvsNM")
OUT_DIR.mkdir(parents=True, exist_ok=True)

VALID_MIN_TOTAL_SPOTS = 2
POSITIVE_RED_THRESHOLD = 8
# =========================================================
# STEP 1.1: Meta grouping config (M vs NM)
# ---------------------------------------------------------
# EN: Define which tumor types belong to NM / M groups.
#     Keep this configurable; edit here when needed.
# CN: 定义 Non-metastatic(NM) 与 Metastatic(M) 分组（可修改）。
#     默认：NM = CRC, GBM；M = PDAC, ESCA；BRCA 暂时忽略（不纳入任何组）。
# =========================================================
META_GROUPS = {
    "NM": ["CRC", "GBM"],
    "M":  ["PDAC", "ESCA"],
}
GROUP_ORDER = ["NM", "M"]  # plotting order

# keep variable name SAMPLE_TYPE_ORDER for minimal code change
SAMPLE_TYPE_ORDER = GROUP_ORDER

# Step style colors (group colors)
GROUP_COLORS = {
    "NM": "#1f77b4",  # blue
    "M":  "#d62728",  # red
}
...
# =========================================================
# STEP 3.5: Map tumor type -> meta group (NM/M), drop others
# ---------------------------------------------------------
# EN: Preserve original tumor type as tumor_type, and overwrite sample_type
#     with meta group label ("NM"/"M") for all downstream steps.
# CN: 保留原始肿瘤类型到 tumor_type，并将 sample_type 覆盖为分组标签（NM/M），
#     以便后续步骤直接复用 v1 的绘图/统计逻辑。
# =========================================================
_TUMOR_TO_META = {t: g for g, ts in META_GROUPS.items() for t in ts}

def add_meta_group(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df
    df = df.copy()
    # preserve tumor type
    if "sample_type" in df.columns and "tumor_type" not in df.columns:
        df.rename(columns={"sample_type": "tumor_type"}, inplace=True)
    if "tumor_type" in df.columns:
        df["sample_type"] = df["tumor_type"].astype(str).map(_TUMOR_TO_META)
    # keep only defined groups (drop BRCA/others by default)
    df = df.loc[df["sample_type"].isin(GROUP_ORDER)].copy()
    return df

per_cell_all = add_meta_group(per_cell_all)
gr_per_cell_all = add_meta_group(gr_per_cell_all)
gr_long_all = add_meta_group(gr_long_all)
...
set_small_axes_with_right_panel(fig, ax, width=0.056, height=0.16)
...
# =========================================================
# STEP 4 (NEW v2.1): ROI-level positive ratio (per sample_id) with M vs NM stats
# ---------------------------------------------------------
# EN: Treat each unique sample_id as one ROI/sample; compute positive ratio per ROI.
#     Compare NM vs M using Welch t-test, and plot violin in Step9-like bracket style.
# CN: 以 sample_id 作为 ROI/样本单位，计算每个 ROI 的阳性比例。
#     NM vs M 做 Welch t-test，并按 Step9 风格绘制 violin + 括号 + pvalue + N(ROI数)。
# =========================================================
def _roi_positive_ratio_df(valid_cells_df: pd.DataFrame) -> pd.DataFrame:
    df = valid_cells_df.copy()
    df["sample_id"] = df["sample_id"].astype(str)
    df["sample_type"] = df["sample_type"].astype(str)
    g = (
        df.groupby(["sample_type", "sample_id"])["is_positive"]
        .agg(n_valid="count", n_pos="sum")
        .reset_index()
    )
    g["pos_ratio"] = g["n_pos"] / g["n_valid"].replace(0, np.nan)
    return g

def _ttest_welch(a: np.ndarray, b: np.ndarray) -> float:
    a = np.asarray(a, dtype=float); b = np.asarray(b, dtype=float)
    a = a[np.isfinite(a)]; b = b[np.isfinite(b)]
    if ttest_ind is None or a.size < 2 or b.size < 2:
        return np.nan
    return float(ttest_ind(a, b, equal_var=False).pvalue)

def _draw_pair_bracket(ax: plt.Axes, x1: float, x2: float, y: float, h: float, text: str, lw: float = 0.8):
    ax.plot([x1, x1, x2, x2], [y, y + h, y + h, y], color="black", lw=lw, clip_on=False)
    ax.text((x1 + x2) / 2.0, y + h, text, ha="center", va="bottom", clip_on=False)

def _plot_roi_violin_m_vs_nm(
    df_roi: pd.DataFrame,
    value_col: str,
    tag: str,
    title: str,
    ...
    # order fixed: NM, M (use existing GROUP_ORDER and GROUP_COLORS)
    groups = [g for g in GROUP_ORDER if g in df_roi["sample_type"].astype(str).unique().tolist()]
    if len(groups) == 0:
        print("[WARN] empty ROI df for", tag)
        return None

    data_by_group = []
    n_by_group = []
    for g in groups:
        arr = pd.to_numeric(df_roi.loc[df_roi["sample_type"].astype(str) == g, value_col], errors="coerce").to_numpy(dtype=float)
        arr = arr[np.isfinite(arr)]
        data_by_group.append(arr)
        n_by_group.append(int(np.sum(np.isfinite(arr))))

    # pvalue (NM vs M)
    pval = np.nan
    if len(groups) == 2:
        pval = _ttest_welch(data_by_group[0], data_by_group[1])

    apply_plot_style()
    fig, ax = plt.subplots(figsize=FIGSIZE)
    ax.set_position(AX_POS)

    colors = [GROUP_COLORS.get(g, "black") for g in groups]
    out_table = out_base / f"{GROUP}_{tag}_roi_violin_data.csv"
    ...
    # bracket + pvalue (Step9-like)
    if len(groups) == 2:
        # get current y range after violin function
        ymin, ymax = ax.get_ylim()
        y_rng = max(ymax - ymin, 1e-9)
        y = ymax - 0.20 * y_rng
        h = 0.02 * y_rng
        _draw_pair_bracket(ax, 1, 2, y, h, f"P={_pval_fmt(pval)}", lw=0.8)

    out_png = out_base / f"{GROUP}_{tag}_roi_violin.png"
    out_pdf = out_base / f"{GROUP}_{tag}_roi_violin.pdf"
    save_fig(fig, out_png, out_pdf)
    print("[OK][Step4 v2.1] ROI violin saved:")
    print(" -", out_png)
    print(" -", out_pdf)
    print("[OK][Step4 v2.1] ROI table saved:")
    print(" -", out_table)
    return out_png, out_table

# ---- run ROI-level positive ratio violin (all valid cells) ----
roi_ratio_df = _roi_positive_ratio_df(valid_cells)
_plot_roi_violin_m_vs_nm(
    roi_ratio_df,
    value_col="pos_ratio",
    tag="positive_ratio_all_valid_cells",
    title=f"{GROUP}: positive ratio per ROI (all valid cells)",
    ylabel="Positive ratio per ROI",
    out_base=OUT_DIR,
)
...
# =========================================================
# STEP 6 (NEW v2.1): ROI-level red copy number (per sample_id) with M vs NM stats
# ---------------------------------------------------------
# EN: For each ROI(sample_id), compute mean red copy number across cells.
#     Compare NM vs M using Welch t-test, and plot violin (all valid / positive only).
# CN: 以 sample_id 为 ROI，计算每个 ROI 的 red 拷贝数均值；
#     NM vs M 做 Welch t-test，并绘制 violin（有效细胞 / 阳性细胞各一张）。
# =========================================================
def _roi_mean_copy_df(cells_df: pd.DataFrame, copy_col: str = "n_spots_red") -> pd.DataFrame:
    df = cells_df.copy()
    df["sample_id"] = df["sample_id"].astype(str)
    df["sample_type"] = df["sample_type"].astype(str)
    df[copy_col] = pd.to_numeric(df[copy_col], errors="coerce")
    g = (
        df.groupby(["sample_type", "sample_id"])[copy_col]
        .mean()
        .reset_index()
        .rename(columns={copy_col: "roi_mean_copy"})
    )
    return g

roi_copy_valid = _roi_mean_copy_df(valid_cells, "n_spots_red")
_plot_roi_violin_m_vs_nm(
    roi_copy_valid,
    value_col="roi_mean_copy",
    tag="red_copy_roi_all_valid_cells",
    title=f"{GROUP}: red copy number per ROI (all valid cells)",
    ylabel=f"{GROUP} spots per nucleus (ROI mean)",
    out_base=OUT_DIR,
)

roi_copy_pos = _roi_mean_copy_df(pos_cells, "n_spots_red")
_plot_roi_violin_m_vs_nm(
    roi_copy_pos,
    value_col="roi_mean_copy",
    tag="red_copy_roi_positive_cells_only",
    title=f"{GROUP}: red copy number per ROI (positive cells only)",
    ylabel=f"{GROUP} spots per nucleus (ROI mean)",
    out_base=OUT_DIR,
)
...
# =========================================================
LABEL_MAP = {
    "red-green": "SEMA3B and CEN3",
    "red-red": "SEMA3B and SEMA3B",
...
# =========================================================
# STEP 8 (NEW v2.1): ROI-level probability of colocalization (per sample_id) with M vs NM stats
# ---------------------------------------------------------
# EN: For each ROI(sample_id), compute mean probability across cells (red-green only).
#     Compare NM vs M using Welch t-test, and plot violin (all valid / positive only).
# CN: 以 sample_id 为 ROI，计算每个 ROI 的 colocalization 概率均值（只比较 red-green）。
#     NM vs M 做 Welch t-test，并绘制 violin（有效细胞 / 阳性细胞各一张）。
# =========================================================
def _roi_mean_prob_df(cells_df: pd.DataFrame, prob_col: str) -> pd.DataFrame:
    df = cells_df.copy()
    df["sample_id"] = df["sample_id"].astype(str)
    df["sample_type"] = df["sample_type"].astype(str)
    df[prob_col] = pd.to_numeric(df[prob_col], errors="coerce")
    g = (
        df.groupby(["sample_type", "sample_id"])[prob_col]
        .mean()
        .reset_index()
        .rename(columns={prob_col: "roi_mean_prob"})
    )
    return g

# Use pro_coloc_red-green (SEMA3B vs CEN3) as primary colocalization metric
roi_prob_valid = _roi_mean_prob_df(valid_cells, "pro_coloc_red-green")
_plot_roi_violin_m_vs_nm(
    roi_prob_valid,
    value_col="roi_mean_prob",
    tag="prob_coloc_red_green_roi_all_valid_cells",
    title=f"{GROUP}: P(coloc red-green) per ROI (all valid cells)",
    ylabel="Probability (ROI mean)",
    out_base=OUT_DIR,
)

roi_prob_pos = _roi_mean_prob_df(pos_cells, "pro_coloc_red-green")
_plot_roi_violin_m_vs_nm(
    roi_prob_pos,
    value_col="roi_mean_prob",
    tag="prob_coloc_red_green_roi_positive_cells_only",
    title=f"{GROUP}: P(coloc red-green) per ROI (positive cells only)",
    ylabel="Probability (ROI mean)",
    out_base=OUT_DIR,
)
...
# =========================================================
# STEP 9 (NEW v2.1): ROI-level g(0) for red experimental channel with M vs NM stats
# ---------------------------------------------------------
# EN: For each ROI(sample_id), compute mean g0_exp for red channel across cells.
#     Compare NM vs M using Welch t-test, and plot violin (all valid / positive only).
# CN: 以 sample_id 为 ROI，计算每个 ROI 的 red 通道 g0_exp 均值；
#     NM vs M 做 Welch t-test，并绘制 violin（有效细胞 / 阳性细胞各一张）。
# =========================================================
def _roi_mean_g0_red_df(gr_df: pd.DataFrame) -> pd.DataFrame:
    df = gr_df.copy()
    df["sample_id"] = df["sample_id"].astype(str)
    df["sample_type"] = df["sample_type"].astype(str)
    df["channel"] = df["channel"].astype(str).str.lower()
    df["g0_exp"] = pd.to_numeric(df["g0_exp"], errors="coerce")
    sub = df.loc[df["channel"] == "red"].copy()
    g = (
        sub.groupby(["sample_type", "sample_id"])["g0_exp"]
        .mean()
        .reset_index()
        .rename(columns={"g0_exp": "roi_mean_g0_red"})
    )
    return g

roi_g0_valid = _roi_mean_g0_red_df(gr_valid)
_plot_roi_violin_m_vs_nm(
    roi_g0_valid,
    value_col="roi_mean_g0_red",
    tag="g0_red_roi_all_valid_cells",
    title=f"{GROUP}: g(0) red per ROI (all valid cells)",
    ylabel="Autocorrelation g(0) (ROI mean)",
    out_base=OUT_DIR,
)

roi_g0_pos = _roi_mean_g0_red_df(gr_pos)
_plot_roi_violin_m_vs_nm(
    roi_g0_pos,
    value_col="roi_mean_g0_red",
    tag="g0_red_roi_positive_cells_only",
    title=f"{GROUP}: g(0) red per ROI (positive cells only)",
    ylabel="Autocorrelation g(0) (ROI mean)",
    out_base=OUT_DIR,
)
...
# 1) Produce TWO outputs with r_plot_max=20 and r_plot_max=40 (all else unchanged).
#    - Each r_plot_max uses its own Step10Params instance
#    - Each r_plot_max writes to its own tag suffix to avoid overwriting
# =========================================================
...
# NEW: create two parameter sets (only r_plot_max differs)
P10_20 = Step10Params(r_plot_max=30)
...
# =========================================================
# 9) Driver usage (NEW: run twice for r_plot_max=20 and 40)
# =========================================================
# IMPORTANT: tags include rmax to avoid overwriting outputs
