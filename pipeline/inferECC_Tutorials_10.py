...
###
ref_file_path = "/jdfssz1/ST_HEALTH/P22Z10101N0003/Project/06.ecDNA/02.code/v0.1.1/fig_100k_obs-ref/11sample_ref/f02-cpr_ref_var.tsv"
###
...
df_fragments_cutoff_normalize_dd.to_csv("f02-cell_chr100k_coverage.tsv",sep="\t",index=True)
del df_fragments_cutoff_normalize_dd
...
df_08.to_csv("f03-coverage_fi_score.tsv",sep="\t",index=True)

del df_08
del df_fragments_cutoff_normalize
del df_fragments_cutoff_segmentation
del df_fragments_cutoff_sample
del df_fragments_cutoff
del df_fragments
del df_08_dd
del df_07_dd
del df_07
...
os.chdir(outfile_path)
print(os.getcwd())
#df_ch = pd.read_csv("./cellXecDNA.matrix.tsv",sep="\t")
#df_ch = df_08.copy()
df_ch = pd.read_csv("./f03-coverage_fi_score.tsv",sep="\t")
...
#### adata_fi_back
reference = pd.read_csv(ref_file_path,sep="\t",index_col=0)
reference_series = reference["cov_mean3std"].copy()
def adata_fi_background(adata, reference):
    # 步骤1: 获取参考序列
...
import warnings
import scanpy as sc
adata_sample = sc.read("cellXecDNA_fi.matrix.h5ad")
adata_sample_rebg = adata_fi_background(adata=adata_sample, reference=reference_series)
adata_sample_rebg.write_h5ad("cellXecDNA_fi.matrix_remove_background_mean3std.h5ad")
...
cell_coverage = pd.read_csv("./f02-cell_chr100k_coverage.tsv",sep="\t")
...
gene_df = pd.read_csv("./f03-coverage_fi_score.tsv",sep="\t")
